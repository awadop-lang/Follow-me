...
<body onload="initApp()">
    <header>
        ...
        <div style="font-family:monospace; font-size:12px; color:var(--cyan);">[ {{ session['user'].upper() }} ] @ <span id="reg-name">---</span></div>
                    ...
                    return `<div class="item watched">
                        ...
                        <div class="log-grid">
                            <div class="time-badge">
                                <span class="time-label">Connection</span>
                                <span class="time-value val-in">${hist.in}</span>
                            </div>
                            <div class="time-badge">
                                <span class="time-label">Disconnection</span>
                                <span class="time-value val-out">${hist.out}</span>
                            </div>
                        </div>
                    </div>`;
                }).join('');
...
def toggle_watch():
    ...
    user = session['user']
    if name in users_db[user]['watchlist']: users_db[user]['watchlist'].remove(name)
    else: users_db[user]['watchlist'].append(name)

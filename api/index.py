...
app = Flask(__name__)
app.secret_key = "NOX_ZETA_TIMEZONE_PRO_V9"

# Base de données multi-utilisateurs
users_db = {
    "admin": {
        "pw": "1234", "region": "OFFLINE", "coords": {"x":0, "y":0}, 
        "avatars": [], "history": {}, "watchlist": [], "tz": "UTC"
    }
}
...
<head>
    ...
    <style>
        ...
        .time-selector { background: #111; border: 1px solid var(--border); color: var(--cyan); font-family: 'Rajdhani'; font-size: 12px; padding: 5px; cursor: pointer; border-radius: 3px; max-width: 200px; }
        ...
        .item { background: rgba(255,255,255,0.02); border: 1px solid var(--border); padding: 15px; margin-bottom: 12px; }
        .item.watched { border-left: 4px solid var(--red); background: rgba(255, 49, 49, 0.05); }
        .name { color: var(--cyan); font-weight: 700; font-size: 16px; font-family: 'Orbitron'; text-decoration: none; }
        ...
        .time-label { font-size: 9px; text-transform: uppercase; color: #888; display: block; }
        .time-value { font-family: 'JetBrains Mono', monospace; font-size: 13px; font-weight: 700; color: #fff; }
        .val-in { color: var(--green); } .val-out { color: var(--red); }
        .action-btn { background: transparent; border: 1px solid var(--cyan); color: var(--cyan); font-family: 'Orbitron'; width: 30px; height: 30px; cursor: pointer; }
...
<body onload="initApp()">
    <header>
        <div class="logo">NOX//ZETA</div>
        <div style="display:flex; align-items:center; gap:10px;">
            <span style="font-size:10px; color:#555;">TIMEZONE:</span>
    ...
    <div class="main-container">
        <div class="column" style="width: 40%;"><div class="col-header">Radar</div><div class="scroll-area" style="display:flex; justify-content:center;"><div class="map-frame"><div id="map-bg"></div><canvas id="radar-canvas" width="512" height="512"></canvas></div></div></div>
        <div class="resizer"></div>
        <div class="column" style="width: 25%;"><div class="col-header">Scanner</div><div id="scan-list" class="scroll-area"></div></div>
        <div class="resizer"></div>
    ...
    <script>
        // Liste complète des zones via Intl
        ...
        async function updateUI() {
            ...
            document.getElementById('scan-list').innerHTML = data.avatars.map(av => `
                <div class="item">
                    <span class="name" style="font-size:13px;">${av.name}</span>
                    <button class="action-btn" onclick="toggleWatch('${av.name}')" style="float:right;">${watchlist.includes(av.name)?'-':'+'}</button>
                </div>`).join('');
            ...
            document.getElementById('watch-list').innerHTML = watchlist.map(name => {
                ...
                <div class="item watched">
                    ...
                    <div class="log-grid">
                        <div class="time-badge"><span class="time-label">Connection</span><span class="time-value val-in">${formatTime(hist.in)}</span></div>
                        <div class="time-badge"><span class="time-label">Disconnection</span><span class="time-value val-out">${formatTime(hist.out)}</span></div>
                    </div>
                </div>`;
            }).join('');
        }
...
def index():
    if request.method == 'POST':
        ...
        if user in users_db:
            now_ts = time.time() # On stocke le TIMESTAMP brut (secondes depuis 1970)

...
<head>
    ...
    <style>
        ...
        .map-wrapper { grid-column: 1; grid-row: 1; border: 1px solid #222; background: #000; position: relative; box-shadow: 0 0 20px rgba(0,0,0,0.5); }
        ...
        .card { background: rgba(255,255,255,0.01); border: 1px solid #1a1a1a; padding: 10px; margin-bottom: 5px; cursor: pointer; font-size: 12px; transition: all 0.2s; }
        .card:hover { border-color: var(--p); background: rgba(0,255,255,0.05); }
        .card.selected { border-left: 4px solid var(--p); background: rgba(0,255,255,0.1); border-color: var(--p); }
        ...
        .inspector { background: #000; border: 1px solid #222; padding: 15px; min-height: 120px; border-top: 2px solid var(--p); display: flex; align-items: flex-start; }
...
<body>
    <header>
        <div style="font-size: 16px; font-weight: bold; letter-spacing: 4px; color: var(--p);">[ TACTICAL_MONITOR_V7.1 ]</div>
        ...
        <div class="watchlist-panel">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                ...
                <div class="watch-input-group">
                    ...
                    <button class="add-btn" onclick="addWatch()">ADD TO TRACKER</button>
                </div>
            </div>
            <div style="overflow-y:auto; flex:1;">
                <table class="w-table">
                    <thead>
                        <tr><th>AGENT / UUID</th><th>STATUS</th><th>ACTION</th></tr>
                    </thead>
    ...
    <script>
        ...
        async function update() {
            try {
                ...
                d.avatars.forEach((av, i) => {
                    ...
                    // Dessin du point sur la carte
                    if (isSelected) {
                        // Cercle pulsant pour la sélection
                        ...
                        // Point normal (légèrement transparent si un autre est sélectionné)
                ...
                Object.keys(d.watchlist).forEach(uuid => {
                    ...
                    let isON = info.online;
                    let c = isON ? "#00ff00" : "#444";
                    let statusText = isON ? "ONLINE" : "OFFLINE";
                    if(isON && info.start > 0) statusText += " (" + Math.floor((Date.now()/1000 - info.start)/60) + "m)";
        ...
        setInterval(update, 200); // Mise à jour plus rapide pour l'animation du point
    </script>
</body>
</html>
"""
...
def add_watch():
    data = request.get_json(silent=True)
    uid = data.get("uuid")
    if uid and uid not in watchlist: watchlist[uid] = {"name": "", "online": False, "start": 0}
    return "OK"

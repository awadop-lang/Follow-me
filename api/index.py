...
# --- INTERFACE TACTIQUE NOX V5.3 (CLICKABLE PROFILES) ---
HTML_CODE = """
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        .card:hover { 
            background: rgba(0,255,255,0.05); border-color: var(--p); 
            transform: translateX(5px); box-shadow: -5px 0 10px rgba(0,255,255,0.1);
        }
        .card:active { transform: scale(0.98); }
        .card::after { 
            content: 'VIEW PROFILE'; position: absolute; right: 10px; bottom: 5px; 
            font-size: 8px; color: var(--p); opacity: 0; transition: opacity 0.2s;
        }
...
<body>
    <header>
        ...
        <div id="status" style="font-size: 10px; opacity: 0.6;">MODE: INTERACTIVE_PROFILING</div>
    </header>
    <div class="grid">
            ...
            try {
                ...
                d.avatars.forEach((av, i) => {
                    ...
                    card.innerHTML = `
                        ...
                        <div style="font-size:9px; margin-top:4px; opacity:0.4;">U_TIME: ${Math.floor(timeS/60)}m ${timeS%60}s | ID: ${av.key.substring(0,8)}...</div>
                    `;

...
app = Flask(__name__)
app.secret_key = "NOX_ZETA_STABLE_V12_5"

# Base de données persistante pour la session (en mémoire)
users_db = {
    "admin": {
...
<body onload="initApp()">
    <header>
        <div class="logo">NOX//ZETA v1.2.5</div>
        <div style="display:flex; align-items:center; gap:10px;">
            <select id="tz-select" class="time-selector" onchange="updateUI()"></select>
            <div style="font-family:monospace; font-size:12px; color:var(--cyan);">[ {{ user_name.upper() }} ]</div>
        </div>
            ...
            try {
                ...
                document.getElementById('scan-list').innerHTML = avatars.map(av => `
                    <div class="item" style="${selectedAgent === av.name ? 'border: 1px solid var(--cyan); background: rgba(0,255,255,0.05);' : ''}">
                        <div>
                            <div><span class="name" onclick="selectedAgent='${av.name}'">${av.name}</span><a href="https://my.secondlife.com/${av.name.replace(/ /g, '.')}" target="_blank" class="profile-link">🔗</a></div>
